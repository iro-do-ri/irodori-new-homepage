        ...
        <section className={styles.container} ref={wrapperRef}>
            ...
            <section ref={textRef_3} className={`${styles.news} sm:absolute relative w-full`}>
              ...
              <ul className={`${styles.newsList} mb-6 sm:mb-12`}>
                          ...
                          <li key={news.id} className="mb-3 sm:mb-4">
                            <article>
                              <Link href={`/news/${news.id}`} className="inline-flex items-start sm:items-center flex-wrap gap-y-1">
                                <span className={`${styles.img} mr-3`}><img src={news.img?.url??"https://images.microcms-assets.io/assets/1c47cf40b1b24139aa6e76b7efe668bc/eaa762c22949424c87cd0aadf582116e/no-image.png"} alt={news.title} loading="lazy" decoding="async"/></span>
                ...
                <Link href={URL.News} className={`${styles.box_button} inline-block`} aria-label="イロドリの更新情報の一覧ページへ移動します"><span className={`${styles.background_button} ${styles.other_button}`}></span><span className={`${styles.contents_button} sm:px-12 px-6 sm:py-6 py-3 inline-block`}>イロドリの更新情報の一覧へ</span></Link>
                </div>
              </ul>
              </div>
              </div>
